...
) ENGINE = OLAP UNIQUE KEY (`session_id`) -- 关键修改：改为 UNIQUE KEY
DISTRIBUTED BY HASH (`session_id`) BUCKETS 10 PROPERTIES (
...
) ENGINE = OLAP DUPLICATE KEY (id) DISTRIBUTED BY HASH (session_id) BUCKETS 10 PROPERTIES ('replication_num' = '1');
...
) ENGINE = OLAP DUPLICATE KEY (`id`) DISTRIBUTED BY HASH (`event_time`) BUCKETS 10 PROPERTIES ('replication_num' = '1');
...
) ENGINE = OLAP UNIQUE KEY (`config_key`) DISTRIBUTED BY HASH (`config_key`) BUCKETS 1 PROPERTIES (
    "enable_unique_key_merge_on_write" = "true",
    "replication_allocation" = "tag.location.default: 1",
    "light_schema_change" = "true"
);
...
CREATE TABLE IF NOT EXISTS `agent_models` (
    ...
    `provider`,
    `model_id`
) DISTRIBUTED BY HASH (`agent_name`, `provider`) BUCKETS 4 PROPERTIES (
    "enable_unique_key_merge_on_write" = "true",
    "replication_allocation" = "tag.location.default: 1",
    "light_schema_change" = "true"
);

-- cron_jobs 表：cron 定时任务数据
CREATE TABLE IF NOT EXISTS `cron_jobs` (
    `id` varchar(128) NOT NULL DEFAULT "" COMMENT "cron jobid",
    `agent_id` varchar(128) NOT NULL DEFAULT "" COMMENT "agent id",
    `session_key` varchar(128) NOT NULL DEFAULT "" COMMENT "session_key",
    `name` varchar(256) NOT NULL DEFAULT "" COMMENT "cron 任务名",
    `log_attributes` variant NOT NULL COMMENT "cron_jobs原始日志"
) ENGINE = OLAP UNIQUE KEY (`id`) DISTRIBUTED BY HASH (`id`) BUCKETS 10 PROPERTIES (
    "enable_unique_key_merge_on_write" = "true",
    "replication_allocation" = "tag.location.default: 1",
    "light_schema_change" = "true"
);

-- cron_runs 表：cron 定时任务明细数据
CREATE TABLE IF NOT EXISTS `cron_runs` (
    `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT "自增id",
    `ts` bigint NOT NULL DEFAULT 0 COMMENT "日志事件产生时间",
    `job_id` varchar(128) NOT NULL DEFAULT "" COMMENT "cron jobid",
    `action` varchar(128) NOT NULL DEFAULT "" COMMENT "任务执行流当前所处的生命周期阶段",
    `status` varchar(128) NOT NULL DEFAULT "" COMMENT "执行状态",
    `delivery_status` varchar(128) NOT NULL DEFAULT "" COMMENT "交付状态",
    `model` varchar(128) NOT NULL DEFAULT "" COMMENT "模型",
    `provider` varchar(128) NOT NULL DEFAULT "" COMMENT "模型提供商",
    `log_attributes` variant NOT NULL COMMENT "cron_runs原始日志"
) ENGINE = OLAP DUPLICATE KEY (`id`) DISTRIBUTED BY HASH (`job_id`) BUCKETS 10 PROPERTIES ('replication_num' = '1');